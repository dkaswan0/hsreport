      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto font-sans dir-rtl text-right">
        <DialogHeader className="text-right border-b pb-3 mb-2">
          <DialogTitle className="flex items-center gap-2 text-xl font-bold text-slate-900 dark:text-white">
            <Globe className="w-6 h-6 text-zinc-950 animate-pulse" />
        ...
        <form onSubmit={handleSearch} className="space-y-4 my-2">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              />
              <Button type="submit" disabled={isLoading} className="bg-zinc-950 hover:bg-black text-white min-w-[120px]">
                {isLoading ? (
                  <>
        ...
        {searchedQuery && (
          <div className="text-xs text-slate-500 bg-zinc-100 dark:bg-zinc-900 p-2.5 rounded-lg border border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
            <span>كلمة البحث الحالية: <strong>"{searchedQuery}"</strong></span>
            <Badge variant="outline" className="bg-zinc-200 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100">Search Router Live API</Badge>
          </div>
        )}
        ...
        <div className="space-y-3 mt-4">
          {isLoading && (
            <div className="py-12 text-center text-slate-500 space-y-3">
              <Loader2 className="w-8 h-8 animate-spin mx-auto text-zinc-950" />
          ...
          {!isLoading && results.length > 0 && (
            <div className="space-y-3">
              ...
              {results.map((item, idx) => (
                <Card key={idx} className="border border-slate-200 dark:border-slate-800 hover:border-zinc-400 transition-colors shadow-sm">
                  <CardContent className="p-4 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-base font-bold text-zinc-950 dark:text-white hover:underline flex items-center gap-1.5 leading-snug"
                      >
                        {item.title}
